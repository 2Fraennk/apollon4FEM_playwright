...
def locate_inputfield_find_owners_last_name(page: Page) -> Locator:
    """ Get locator for the input field last name on page 'find owners' """
    logger.info("Start to find input field.")
    inputfield_find_owners_last_name = page.get_by_role("textbox")
    if inputfield_find_owners_last_name:
        inputfield_find_owners_last_name.highlight()
        time.sleep(1)
        logger.info("Found input field.")
        return inputfield_find_owners_last_name
    else:
        logger.exception("EXCEPTION : inputfield_find_owners_last_name not found.")
        return None
...
def count_locate_button_add_owner(page: Page) -> Locator:
    """ Get locator for the button 'add owners' """
    button_add_owner = page.get_by_role("button").get_by_text("Add Owner", exact=True)
    button_add_owner.highlight()
    time.sleep(1)
